...
**Nothing is delivered locally unless `RESEND_API_KEY` is set.** `emails_enabled`
requires both `RESEND_API_KEY` and `EMAILS_FROM_EMAIL`, and without them sends
are skipped. The API still reports success either way — `POST /password-recovery/{email}`
...
the stack, but nothing has sent to it since the move to Resend, so it was
